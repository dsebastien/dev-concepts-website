...
const GENERIC_ERROR =
  "There was an error subscribing you to the newsletter. Send me an email at sebastien@developassion.be and I'll add you to the list.";
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (!GHOST_API_KEY) {
      throw new Error('The following environment variable is mandatory: [GHOST_API_KEY]');
    }
    const GHOST_API_URL = process.env.GHOST_API_URL;
    if (!GHOST_API_URL) {
      throw new Error('The following environment variable is mandatory: [GHOST_API_URL]');
    }
    ...
    try {
      ...
      console.log('Error while subscribing a new user to the newsletter: ', err);
