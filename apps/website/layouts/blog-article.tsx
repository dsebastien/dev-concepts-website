        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-full mt-2 mb-8">
          <div className="flex items-center">
            <a href="https://twitter.com/dSebastien" className="rounded-full">
              <Image alt="Dubois Sébastien" height={24} width={24} src="/avatar.jpeg" className="rounded-full" />
            </a>
            <p className="text-sm text-gray-700 dark:text-gray-300 ml-2">
              {`${frontMatter.author ? frontMatter.author : AUTHOR_MICRODATA.name} / `}
              <time dateTime={format(parseISO(frontMatter.publishedAt), 'yyyy-MM-dd')}>
                {format(parseISO(frontMatter.publishedAt), 'MMMM dd, yyyy')}
              </time>
            </p>
          </div>
          <p className="text-sm text-gray-500 mt-2 md:mt-0">{frontMatter.readingTime?.text}</p>
        </div>
